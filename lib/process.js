...
var translateDepVersion = function(ebuilder, name, range) {
   ...
   if (range == "*") {
      ...
      ans = ebuild.getValidRange4Version(ebuild.category, name, range);
...
var prepareEbuildDependencies = function(ebuilder, pkg) {
   ...
   for (idx in pkg.dependencies) {
      ...
      if (pkg.dependencies[idx].direct && systemDep) {
   ...
   for (idx in pkg.dependencies) {
      ...
      if ((!isPresent) && systemDep) {
         ...
         if ("availableVersion" in p) {
            pkg2Elaborate.version = p.availableVersion;
         } else {
            ...
            pkg2Elaborate.version = pj.version;
         }
         ...
         ebuilder.addPackage(pkg2Elaborate);
