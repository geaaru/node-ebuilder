...
var getDependenciesListByModules = function(pkg) {
   ...
   for (idx in dirs) {
      ...
      if (n.substring(0, 1) != '.' && !n.startsWith('@')) {
         // POST: Is not a hidden directory and is not a sub-directory
         //       dependency.
...
var prepareEbuildDependencies = function(ebuilder, pkg) {
   ...
   for (idx in pkg.dependencies) {
      ...
      if (pkg.dependencies[idx].direct && systemDep && notExclude) {
         ...
         // For now subproject dependencies are managed with NPM_LOCAL_MODULES
         // variable.
         if (pkg.dependencies[idx].name.startsWith('@')) {

            logging.Logger.debug(sprintf(
               '[%s] For package %s-%s: For dependency %s force use of NPM_LOCAL_MODULES.',
               'prepareEbuildDependencies', pkg.dependencies[idx].name,
               (pkg.version ? pkg.version : "N.A.")));

            pkg.ebuildFile.addNpmLocalModule(pkg.dependencies[idx].name);
...
var inQueuePkgDeps4Processing = function(ebuilder, pkg) {
   ...
   for (idx in pkg.dependencies) {
      ...
      // Exclude elaboration of sub-directory
